...
test.describe("Informational modal", () => {
    ...
    test("includes What's New and the GitHub link in the updated introduction", async ({ page }) => {
        ...
        await expect.poll(async () => {
            const introduction_size = Number.parseFloat(
                await first_paragraph.evaluate((element) => getComputedStyle(element).fontSize));
            const changelog_size = Number.parseFloat(
                await changelog.evaluate((element) => getComputedStyle(element).fontSize));
            return changelog_size < introduction_size;
        }).toBe(true);
    });
